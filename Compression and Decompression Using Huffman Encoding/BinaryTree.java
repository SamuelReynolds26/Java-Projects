...
public class BinaryTree<E> {
    ...
    public boolean isFull(BinaryTree currTree){
        if (currTree.hasRight() && currTree.hasLeft()){
            return true;
        } else{
            return false;
        }
    ...
    public static boolean isCompleteBinaryTree(BinaryTree tree){
        SLLQueue<BinaryTree> treeQueue = new SLLQueue<>();
        treeQueue.enqueue(tree);
        while (!treeQueue.isEmpty()){
            try{
                BinaryTree currTree = treeQueue.dequeue();
                if (currTree.hasRight() && !currTree.hasLeft()){
                    return false;
                }
                if (currTree.hasLeft() && !currTree.hasRight()){
                    if (!treeQueue.front().isLeaf()){
                        return false;
                    }
                }
                if (currTree.isFull(currTree)){
                    treeQueue.enqueue(currTree.left);
                    treeQueue.enqueue(currTree.right);
                }
            } catch(Exception e){
                System.out.println("treeQueue empty.");
            }
        }
        return true;
    }
